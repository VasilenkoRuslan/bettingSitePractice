<?php
/**
 * Various functions required for betting-foundation to work properly
 *
 * @package betting
 */
 ...
 */
if ( ! class_exists( 'betting_Navwalker' ) ) :
	/**
	 * betting_Navwalker
	 */
	class betting_Navwalker extends Walker_Nav_Menu {
		/**
		 * start_lvl
		 *
	...
	if ( ! class_exists( 'betting_Mobile_Navwalker' ) ) :
		/**
		 * betting_Mobile_Navwalker
		 */
		class betting_Mobile_Navwalker extends Walker_Nav_Menu {
			/**
			 * start_lvl
			 *
			 * @param string  $output output
 ...
 */
if ( ! function_exists( 'betting_menu_fallback' ) ) :
	/**
	 * betting_menu_fallback
	 *
	 * @return void
	 */
	function betting_menu_fallback() {
...
if ( ! function_exists( 'betting_active_nav_class' ) ) :
	/**
	 * betting_active_nav_class
	 *
	 * @param array $classes classes
	 * @param mixed $item item
	 * @return array $classes classes
	 */
	function betting_active_nav_class( $classes, $item ) {
		if ( 1 === $item->current || true === $item->current_item_ancestor ) {
			$classes[] = 'active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'betting_active_nav_class', 10, 2 );
endif;

// 3 - Pagination: {@link https://codex.wordpress.org/Pagination}
if ( ! function_exists( 'betting_pagination' ) ) :
	/**
	 * betting_pagination
	 *
	 * @return void
	 */
	function betting_pagination() {
		global $wp_query;
 ...
 */
if ( ! function_exists( 'betting_active_list_pages_class' ) ) :
	/**
	 * betting_active_list_pages_class
	 *
	 * @param mixed $input input
	 */
	function betting_active_list_pages_class( $input ) {
		$pattern = '/current_page_item/';
		$replace = 'current_page_item active';
		$output  = preg_replace( $pattern, $replace, $input );
		return $output;
	}
	add_filter( 'wp_list_pages', 'betting_active_list_pages_class', 10, 2 );
endif;

// 4 - Comments tree: @link {https://codex.wordpress.org/Function_Reference/Walker_Comment}
if ( ! class_exists( 'betting_Comments' ) ) :
	/**
	 * betting_Comments
	 */
	class betting_Comments extends Walker_Comment {
		/**
		 * Tree type.
		 *
		 * @var string
